...
def get_dataset_iterator():
    ...
    dataset = load_dataset("de-Rodrigo/merit", "en-render-seq", split="test", streaming=True)
...
def compute_output(donut_m, donut_p, evaluator, pixels, image, gt):
    ...
    decoder_input_ids = donut_p.tokenizer(task_prompt, add_special_tokens=False, return_tensors="pt")["input_ids"]
    ...
    sequence = sequence.replace(donut_p.tokenizer.eos_token, "").replace(donut_p.tokenizer.pad_token, "")
...
def process_dataset(dataset_iterator):
    ...
    for i, sample in enumerate(dataset_iterator):
        ...
        # Get output
        output_seq, score = compute_output(model, processor, evaluator, pixel_values, image, gt)
